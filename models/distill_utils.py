...
class NonLocalBlockND(nn.Module):
    ...
    def forward(self, x):
        ...
        phi_x = self.phi(x).view(batch_size, self.inter_channels, -1)       #   2 , 128 , 300 x 300
        f = torch.matmul(theta_x, phi_x)    #   2 , 300x300 , 150x150
        N = f.size(-1)  #   150 x 150
        f_div_C = f / (N + 1e-6) #  add epsilon because of NAN
...
class ViT(nn.Module):
    ...
    def forward(self, img):
        ...
        cls_tokens = repeat(self.cls_token, '() n d -> b n d', b = b)           # 1 x n x d -> (number of token) x n x d
