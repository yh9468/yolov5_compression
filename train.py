...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if opt.distill:
        ...
        teacher = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors'), teacher=True).to(device)  # create
        ...
        if opt.resume:
            ...
            model = Model(cfg or ckpt['model'].yaml, ch=3, nc=nc, anchors=hyp.get('anchors'), hyp=hyp).to(device)  # create
            state_dict = ckpt['model'].float().state_dict()  # to FP32
            state_dict = intersect_dicts(state_dict, model.state_dict(), exclude=exclude)  # intersect
            model.load_state_dict(state_dict, strict=False)  # load
            logger.info('Transferred %g/%g items from %s' % (len(state_dict), len(model.state_dict()), opt.resume))  # report
            print("LOAD RESUME WEIGHT FINISH!")
        else:
            model = Model(cfg, ch=3, nc=nc, anchors=hyp.get('anchors'), hyp=hyp).to(device)  # create
            exclude.append('Detect')
            student_ckpt = torch.load("./baseline/yolov5x.pt", map_location=device)
            s_state_dict = student_ckpt['model'].float().state_dict()

            s_state_dict = intersect_dicts(s_state_dict, model.state_dict(), exclude=exclude)  # intersect
            model.load_state_dict(s_state_dict, strict=False)
            print("LOAD STUDENT FINISH")
    ...
    train_path = [os.path.join(data_dict['train'], txts) for txts in os.listdir(data_dict['train'])]
    test_path = [os.path.join(data_dict['val'], txts) for txts in os.listdir(data_dict['val'])]
    ...
    # DDP mode
    if cuda and RANK != -1:
        teacher = DDP(teacher, device_ids=[LOCAL_RANK], output_device=LOCAL_RANK, find_unused_parameters=True)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        logger.info(('\n' + '%17s' * 9) % ('Epoch', 'gpu_mem', 'box', 'obj', 'cls', 'kd_feat',
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with amp.autocast(enabled=cuda):
                if opt.distill:
                    teacher_info = teacher(imgs)
                    pred, kd_loss = model(imgs, teacher_info=teacher_info, targets=targets)  # forward
                    kd_loss_items = torch.mean(kd_loss.view(1,-1), 1)
                ...
                if opt.distill:
                    loss = loss * 0.5 + kd_loss * 0.5
                    total = (loss_items[-1] * 0.5 + kd_loss_items[-1] * 0.5).unsqueeze(0)
                    loss_items = torch.cat((loss_items[:-1], kd_loss.unsqueeze(0), total))
...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights', type=str, default='baseline/weights/best.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default='models/yolov5x.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default='data/train_test.yaml', help='dataset.yaml path')
    parser.add_argument('--hyp', type=str, default='baseline/hyp.yaml', help='hyperparameters path')
